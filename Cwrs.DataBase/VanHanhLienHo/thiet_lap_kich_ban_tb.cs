//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Cwrs.DataBase.VanHanhLienHo
{
    using System;
    using System.Collections.Generic;
    
    public partial class thiet_lap_kich_ban_tb
    {
        public int thiet_lap_kich_ban_code { get; set; }
        public string thiet_lap_kich_ban_name { get; set; }
        public int vi_tri_ref { get; set; }
        public double muc_nuoc_ho { get; set; }
        public double luu_luong_ve_ho { get; set; }
        public string created_by { get; set; }
        public Nullable<System.DateTime> created_at { get; set; }
        public string last_modified_by { get; set; }
        public Nullable<System.DateTime> last_modified_at { get; set; }
        public int user_ref { get; set; }
        public Nullable<int> deleted_status { get; set; }
        public string row_id { get; set; }
        public Nullable<System.DateTime> thoi_gian { get; set; }
    }
}
