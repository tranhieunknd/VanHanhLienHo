...
var Main = function() {
	...
	var settingsHandler = function() {
		...
		$('#skin_color').attr("href", "../Lib/assets/css/themes/" + appSetting.theme + ".css");
		...
		$('input[name="setting-theme"]').change(function() {
			...
			$('#skin_color').attr("href", "Lib/assets/css/themes/" + selectedTheme + ".css");
		...
		function switchLogo(theme) {
			switch (theme) {
				...
				case "theme-5":
				case "theme-6":
					$(".navbar-brand img").attr("src", "../Lib/assets/images/logo2.png");
					break;

				default:
				    $(".navbar-brand img").attr("src", "../Images/logopctt.png");
					break;
			}
		}
		...
		function defaultSetting() {
			...
			$('#skin_color').attr("href", "~/Lib/assets/css/themes/theme-1.css");
			$(".navbar-brand img").attr("src", "../Images/logopctt.png");
...
// Click đóng menu
$('#app').on('click', 'a.sidebar-mobile-toggler', function () {
    if ($("#app").hasClass("app-sidebar-closed")) {
        $("#logo-title").show();
    }
    else {
...
function AddBreadCrumb(id, name, chucnang) {
    RemoveBreadCrumb(id + 1);
    $("#libreadcrumb" + id).show();
    $("#ibreadcrumb" + id).show();
    $("#abreadcrumb" + id).html(name);
    if (chucnang != undefined) {

        $("#abreadcrumb" + id).attr("onclick", chucnang + "RemoveBreadCrumb(" + (id + 1) + ");");
    }
    else {
        $("#abreadcrumb" + id).attr("onclick", "RemoveBreadCrumb(" + (id + 1) + ");");
    }
}
...
function RemoveBreadCrumb(id) {
    for (var i = id; i < 6; i++) {
        try {
            ...
            $("#ibreadcrumb" + i).hide();
            $("#abreadcrumb" + i).empty();
            $("#abreadcrumb" + i).removeAttr("onclick");
        } catch (e) {
