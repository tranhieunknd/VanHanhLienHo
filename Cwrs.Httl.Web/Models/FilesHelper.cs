...
namespace Cwrs.Httl.Web.Models
{
    public class FilesHelper
    {
       ...
       //khai bao entity van hanh lien ho de luu du lieu
        upload_file_tb db = new upload_file_tb();
        ...
        public void UploadAndShowResults(HttpContextBase ContentBase, List<ViewDataUploadFilesResult> resultList)
        {
            ...
            String fullPath = Path.Combine(StorageRoot);//Dua du lieu vao database
        ...
        private void UploadWholeFile(HttpContextBase requestContext, List<ViewDataUploadFilesResult> statuses)
        {
            ...
            for (int i = 0; i < request.Files.Count; i++)
            {
                ...
                string name = ReadNameFile(file.FileName);
                //var fullPath = Path.Combine(pathOnServer, Path.GetFileName(file.FileName));//luu file vao server
                var fullPath = Path.Combine(pathOnServer, Path.GetFileName(name));
                file.SaveAs(fullPath);//luu file vao server
                //Create thumb
                string[] imageArray = file.FileName.Split('.');//tach lay duoi file
                ...
                statuses.Add(UploadResult(file.FileName, file.ContentLength, name));
            }
        }
        ...
        public ViewDataUploadFilesResult UploadResult(String FileName,int fileSize,String FileFullPath)//suu ham create at
        {
            ...
            var result = new ViewDataUploadFilesResult()
            {
                ...
                size = fileSize,
                type = getType,
                url = UrlBase + ReadNameFile(FileName),
